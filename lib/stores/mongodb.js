    ...
    */
    ...
    create_index: function(attr, type) {
        ...
        var fields = _.reduce(attr, function(memo, key) {
            memo[key] = (type === 'hashed') ? 'hashed' : 1;
            return memo;
        }, {});

        switch(type) {
            case indextypes.COMPOUND:
            case indextypes.HASHED:
                this.collection(function(coll) {
                    coll.ensureIndex(fields);
                });
                break;

            case indextypes.UNIQUE:
                this.collection(function(coll) {
                    coll.ensureIndex(fields, { unique: true });
                });
                break;

            case indextypes.SPARSE:
                this.collection(function(coll) {
                    coll.ensureIndex(fields, { sparse: true });
                });
                break;
        }
    },
