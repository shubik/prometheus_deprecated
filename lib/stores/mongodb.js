...
@class MongodbStore
@constructor
@param {Object} options So far contains name of MongoDB collection corresponding to the model. It may have db connection data as well.
*/

MongodbStore = function(options) {
    if (!options.client || !options.collection) {
        throw new Error('heraclitus MongodbStore(): invalud arguments');
    }

    this.client = options.client;
    this.collection = options.collection.toLowerCase();
}
...
_.extend(MongodbStore.prototype, {
    ...
    */
    ...
    query: function(model) {
        if (model.id instanceof ObjectID) {
            return { _id: model.id }
        } else if (model.id && (model.id.length === 12 || model.id.length === 24)) {
            return { _id: new ObjectID(model.id) };
        } else {
            return null;
        }
    },
    ...
    */
    ...
    update: function(id, data) {
        ...
        this._get_collection()(function(collection) {
            collection.update(query, { $set: data }, { safe: true }, function(err) {
                if (err) {
                    def.resolve(err);
                } else {
                    def.resolve(data);
    ...
    */
    ...
    destroy: function(id, single) {
        ...
        this._get_collection()(function(collection) {
            collection.remove(query, { single: single }, function(err) {
                if (err) {
                    def.resolve(err);
                } else {
                    def.resolve();
    ...
    */
    ...
    /**
    Allows to use a MongoDB collection

    @method _get_collection
    @private
    @return {Functoon} Returns a promise that is resolved with MongoDB collection or connection error
    */
