...
Expected options are:
    store               - instance of a store (e.g. a MongoDB store)
    schema              - rules describing a model: default, validate, permitted, unique
        name            - friendly name for this attribute
        default         - default attribute value
        type            - data type from list of constants in ModelFactory.types
        [validate]      - fn that validates attribute value
        [permitted]     - array with permitted values
        [unique]        - true if value must be unique
        [content_type]  - comma separated mime types, default `text/plain`
        [multiline]     - true if this string is text
        [maxlength]     - number if string has a max length
        [maketag]       - fn that returns HTML tag for this field, by usign value of the attribute
        [readonly]      - true if user is not allowed to change value
    [query]             - fn that returns a query used to initially load a model
    [prototype_methods] - hash with methods that will be added to model constructor's prototype
    [static_methods]    - hash with static methods that will be added to model's constructor
    [hooks]             - hash with callbacks to be executed on model lifecycle events
        ...
        parseForm: function(req) {
            ...
            deferred.map(files, function (file) {
                ...
                fs.readFile(file.path, function (err, data) {
                    var fname    = MD5(file.name + utils.now()) + '_orig.' + file.name.split('.').pop(),
                        path     = __appdir + '/public/uploads/' + fname,
                        path_pub = '/uploads/' + fname;
                    ...
                    if (err) {
                        ...
                        fs.writeFile(path, data, function (err) {
                            if (err) {
                                ...
                                self._attributes[file.attr] = path_pub;
                                _def.resolve(path);
                            }
                        });
                    }
